...
需要注意使用Kinect v2相机采集数据时应在光照条件良好的情况下
这是因为光照较暗时rgb摄像头锁定15帧
"""
...
class KinectDataset(object):
    ...
    def get_last_color_data(self):
        if self._kinect.has_new_color_frame():
            ...
            # 取出彩色图像数据
            color_data = reshape_frame[:, :, 0:3]
        else:
            color_data = None
            color_time = None
        return color_data, color_time
    ...
    def get_last_depth_data(self):
        if self._kinect.has_new_depth_frame():
            ...
            depth_data = reshape_frame[:, :, :]
        else:
            depth_data = None
            depth_time = None
        return depth_data, depth_time
